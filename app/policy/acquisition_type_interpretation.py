...
_LAND_SITE_ACQUISITION_PRINCIPLES = (
    SignalCommercialPrinciple(
        ...
        strengthens_when="Paired with no identified implementation activity and no clear developer-led delivery indication yet - the window may still be genuinely open.",
        weakens_when="Paired with implementation_activity_evidence_identified PRESENT for the same scope, or a clear developer-led delivery indication with no ownership/control uncertainty.",
        neutral_when="Isolated, with no other transaction/disposition signal yet available.",
        distinguishing_evidence="implementation_activity_evidence_identified; ownership_or_control_evidence_changed; the identity/type of any developer indication.",
    ),
    SignalCommercialPrinciple(
        signal="no_qualifying_progress_evidence_identified",
        why_it_matters="This is the closest buyer-independent proxy PropertyAIgent has for 'a covered planning-evidence search found no qualifying implementation activity for this scope' - it is not evidence of unwillingness to build or to sell, and not evidence the site is uncommenced or inactive.",
        ...
        distinguishing_evidence="has_phasing_evidence; approaching_implementation_deadline; ownership_or_control_evidence_changed; coverage_checked_at.",
    ),
    SignalCommercialPrinciple(
        signal="implementation_activity_evidence_identified",
        why_it_matters="Authoritative post-permission planning activity (e.g. a condition-discharge or variation/amendment filing) usually signals the current holder is progressing, not disposing of, THIS scope - though it is evidence of planning activity potentially relevant to implementation, never proof physical works have started.",
        strengthens_when="A distinct, evidenced later phase or adjacent parcel within the same site remains untouched by this activity (see app.reporting.opportunity_intelligence_packet's own linked-context fields) - the land-acquisition interest may lie there instead.",
        weakens_when="The activity covers the same scope the buyer is actually assessing (scope_verified is True).",
        neutral_when="The opportunity's own scope relative to the activity is not yet verified (scope_verified is False) - see wider_site_implementation_activity_context for the preserved wider-site finding.",
        distinguishing_evidence="scope_verified; wider_site_implementation_activity_context; phase_code; linked_strategic_allocation_id.",
    ),
    SignalCommercialPrinciple(
        signal="approaching_implementation_deadline",
        why_it_matters="A statutory deadline approaching without identified progress can mean a holder may welcome a land-acquisition conversation before the permission lapses, or may be about to commence to preserve it - both remain live possibilities.",
        strengthens_when="Paired with no_qualifying_progress_evidence_identified PRESENT and no clear developer-led delivery signal.",
        weakens_when="Paired with implementation_activity_evidence_identified PRESENT (activity to preserve the permission may already be underway).",
        neutral_when="Isolated.",
        distinguishing_evidence="implementation_activity_evidence_identified; ownership_or_control_evidence_changed.",
    ),
    SignalCommercialPrinciple(
...
_STRATEGIC_LAND_CONTROL_PRINCIPLES = (
    SignalCommercialPrinciple(
        signal="implementation_activity_evidence_identified",
        why_it_matters="Can mean the wider strategic allocation has genuinely moved past a control-stage opportunity into delivery - or can describe only a narrower phase that does not represent the whole allocation. Authoritative planning activity, never itself proof physical works have started.",
        strengthens_when="Never strengthens this acquisition type on its own.",
        weakens_when="The evidence is confirmed to apply at the SAME scope as the strategic opportunity being assessed (scope_verified is True) - this is exactly why app.policy.buyer_matching's own STRATEGIC_LAND_CONTROL hard-rejection rule requires verified scope before treating this as disqualifying.",
        neutral_when="scope_verified is False - a confirmed activity signal on a phase, recent permission, or long-pending application does not itself prove the wider allocation has been overtaken (see wider_site_implementation_activity_context for the preserved wider-site finding).",
        distinguishing_evidence="scope_verified; wider_site_implementation_activity_context; whether the opportunity itself is STRATEGIC_LAND or a linked PLANNING_DELIVERY phase.",
    ),
    SignalCommercialPrinciple(
        signal="ownership_or_control_evidence_changed",
...
_AFFORDABLE_HOUSING_PACKAGE_PRINCIPLES = (
    SignalCommercialPrinciple(
        signal="implementation_activity_evidence_identified",
        why_it_matters="Can mean the affordable units are on a concrete, fundable delivery track, or that they have already been allocated to another party (e.g. a registered provider already in place). Authoritative planning activity, never itself proof physical works have started.",
...
_DEVELOPMENT_HOMES_ACQUISITION_PRINCIPLES = (
    SignalCommercialPrinciple(
        signal="implementation_activity_evidence_identified",
        why_it_matters="Positive evidence of an authoritative post-permission planning filing - the read most favourable to this acquisition type of any signal in the taxonomy, though it is evidence of activity potentially relevant to implementation, never itself proof physical works or delivery have started.",
        strengthens_when="Present, for the same scope the buyer is assessing (scope_verified is True).",
        weakens_when="Never weakens this acquisition type - identified implementation activity is inherently a positive-leaning signal here.",
        neutral_when="UNKNOWN (no qualifying activity identified, or scope unverified) rather than confirmed absent - this is genuinely neutral, not negative (see no_qualifying_progress_evidence_identified's own principle below for why it is NOT automatically read negatively even here). Raw EPC/build-status context (app.reporting.opportunity_transaction_signals.TransactionSignals.raw_build_status), where available, is secondary/corroborating only - never itself the basis for this signal.",
        distinguishing_evidence="raw_build_status (secondary/corroborating only); scope_verified; wider_site_implementation_activity_context.",
    ),
    SignalCommercialPrinciple(
        signal="no_qualifying_progress_evidence_identified",
        why_it_matters="May indicate a slower-moving pipeline - a genuine, if evidentially soft, negative for this acquisition type specifically, in contrast to LAND_SITE_ACQUISITION where the same signal opens a land-acquisition window. Never itself evidence the site is uncommenced, inactive, stalled, or dormant.",
        strengthens_when="Never strengthens this acquisition type.",
        weakens_when="Paired with a recent_permission that is no longer within the recency window (i.e. this reads more meaningfully once enough time has passed to plausibly expect activity, never merely because the signal is PRESENT for a very fresh grant).",
        neutral_when="Paired with a still-recent grant (recent_permission PRESENT) - too early to read this as a slow-moving pipeline.",
        distinguishing_evidence="recent_permission; approaching_implementation_deadline; coverage_checked_at.",
    ),
    SignalCommercialPrinciple(
        signal="recent_permission",
        ...
        neutral_when="Always neutral in isolation - this acquisition type cares about delivery activity, which recent_permission alone does not yet establish.",
        distinguishing_evidence="implementation_activity_evidence_identified; raw_build_status (secondary/corroborating only).",
    ),
)
