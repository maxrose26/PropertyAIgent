...
## Pre-merge semantic hardening amendment

Product Owner review before merge identified two deterministic-reporting semantics needing hardening. Both are now fixed at the context/domain layer, not left to future prompt wording.

**Capacity range aggregation.** The original `AllocationReportAggregates.capacity_known_total` summed every entry's `capacity_value` regardless of `capacity_kind` — for a "range" entry (e.g. "8,400–15,000 homes"), `capacity_value` is `format_capacity`'s own existing upper-bound convention (15,000), so that total silently blended genuinely-exact figures with range upper bounds under one ambiguous name. Fixed by replacing `capacity_known_total`/`capacity_unknown_count` with `exact_capacity_total`/`exact_capacity_count` (summing only entries whose `capacity_kind` is `minimum`/`maximum`/`indicative` — a single genuinely-stated figure) plus separate `ranged_capacity_count`/`unknown_capacity_count`. There is deliberately no field left that could be read as "total capacity including ranges" — a future PDF/AI consumer wanting that would have to compute and label it explicitly itself, never inherit that framing silently. `identified_application_capacity`/`indicative_residual_capacity` were audited and found to carry no equivalent range concept at all (`compute_development_coverage` only ever returns a single genuine integer or `None`, never a range) — their existing known-total/unknown-count pairs were already correct, confirmed by a new aggregate-level test proving a `REVIEW_REQUIRED` allocation's `None` residual is excluded from the sum, not summed as zero.

**Review-pending party evidence.** `OwnershipEvidenceEntry.needs_review` already existed, but nothing forced a consumer to check it — the Shortlist page's "Developer:" caption and the CSV's "Known Developer(s)" column both included `needs_confirmation` `ControlRelationship` evidence indistinguishably from trusted evidence (the CSV did already append a "(needs confirmation)" suffix, but within the same nominally-trusted column). Fixed by adding `AllocationReportEntry.trusted_ownership_evidence`/`review_pending_ownership_evidence` properties (partitioning `ownership_evidence` by `needs_review`), so a consumer no longer has to remember to filter it itself. "Known Developer(s)" in CSV now reads only from `trusted_ownership_evidence` — a `needs_confirmation` Developer never appears there, however worded; it remains visible, explicitly qualified, in the general "Ownership / Control Evidence" column. The Shortlist page's "Developer:" caption is now trusted-only, with pending Developer evidence shown on a separate, plainly-worded, unbolded line ("Developer evidence pending confirmation: …") rather than sharing the confident-sounding caption. Rejected evidence remains excluded from both partitions, unchanged. Applicant evidence is unaffected (it has no `needs_review` concept — it is a portal scrape, not `ControlRelationship` evidence).

15 new tests cover both fixes (8 capacity, 7 party trust) plus one structural test confirming the old ambiguous aggregate field names no longer exist at all. Query count re-measured unchanged (flat 9 queries at 5/10/25/50 allocations) — the amendment is pure Python aggregation/formatting logic, no new queries.
