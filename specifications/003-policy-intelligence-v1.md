...
## 9. Validation: Stockport vs Bury (Sprint 2, "Greater Manchester Policy Intelligence Framework")

Bury was onboarded as the second authority to prove the framework generalises, not just to add a second council's data. Both councils' real documents were read and ingested for real - nothing in this section is simulated.

**Terminology.** Stockport's plan status was "draft" throughout the pilot; Bury's own Local Plan calls its Regulation 19 stage "**Publication**" (e.g. "Publication Local Plan", "Publication stage") without necessarily saying "Regulation 19" alongside it. `app.policy.status`'s keyword table had no entry for this - confirmed a real, live normalisation gap, fixed by adding `"publication" -> "proposed_submission"`, checked before the generic "draft" keyword since "Publication Draft Local Plan" phrasing also exists and Publication is the further-along, more specific signal of the two.

**Document structure - the most significant structural difference found.** Stockport's own Local Plan document contains its own self-authored site-allocations schedule (the pilot's whole premise). **Bury's own Local Plan contains none at all** - confirmed independently via direct primary-source reading (fetching the council's own page, and a full-document `pdfplumber` search for allocation-schedule language), not just a search-engine summary. Bury's strategic housing sites were allocated years earlier through a separate, adopted, nine-borough joint plan (*Places for Everyone*, adopted 21 March 2024) that Bury's own document only narratively cross-references. This is not a framework bug to route around - both documents were ingested as genuinely distinct `LocalPlan` rows for the `bury` council (`LocalPlan.council_code` + `plan_name` already supports more than one plan per council from Sprint 1), each with its own real status, version, and source. A future specification should still consider a proper many-to-many Council<->LocalPlan relationship for genuinely joint/multi-authority plans (see the V1 site-link limitation note in §5) - not built in this sprint, since a single joint plan ingested once, correctly attributed to the borough it was ingested *for*, was sufficient to onboard Bury honestly.

**Allocation references.** Stockport uses a "HOM 2.XX" numbering convention; Bury/Places for Everyone uses "JPA N". No normalisation was needed or attempted here - `policy_reference` is deliberately free text (Sprint 1), and different plans legitimately using different coding conventions is expected, not a defect.

**Extraction quality - a real fabrication bug found and fixed.** The first real Bury ingestion run produced five allocations with plausible-looking "HOM 2.3X" policy references - Stockport's own numbering convention, not Bury's. Investigation (reading the exact source text fed to the model) confirmed the source document names these sites in prose ("PfE identifies several strategic housing allocations... at Seedfield, Walshaw, Elton Reservoir...") with **no code printed against them at all** - the model, facing a schema that required `policy_reference` as a non-null string, filled it by copying this very prompt's own example format rather than inventing from nothing, but it was still fabrication: a code that does not appear anywhere in the source. Fixed at the root, not patched around: `policy_reference` is now nullable end-to-end (extraction schema, `LocalPlanSite`, `AllocationVersion`, plus a SQLite table rebuild for the two already-existing tables - `ALTER TABLE ADD COLUMN` cannot loosen an existing NOT NULL constraint, a real instance of the schema-migration limitation flagged in the Sprint 1 CTO review), the prompt now explicitly forbids inventing, inferring, or reusing a code, and - a second, related fix - the prompt now also distinguishes a document's **own** allocations from sites it merely *references* as allocated by a different plan, so a narrative cross-reference is no longer extracted as if this document were the allocating authority. Re-run with both fixes, the same five sites correctly extracted with `policy_reference = null` (honest) and zero fabrication; the two sites Bury's own document DOES cite by policy number (JPA1.1/JPA1.2, Northern Gateway) were correctly extracted with their real codes. **Source fidelity was not reduced - it was corrected.**

**Adopted-status safeguard, validated on real adopted-plan data.** *Places for Everyone* is a genuinely adopted plan. Its JPA7/JPA8/JPA9 allocations (Elton Reservoir 3,500 homes, Seedfield 140, Walshaw 1,250 - real figures, verified against the primary policy text, not a search summary) correctly show `progression_signal = "unknown"`, not `"adopted"`, because their own allocation-level status has no independent confirmation beyond the plan being adopted (the Sprint 1 CTO-review safeguard - §5, "an adopted LocalPlan must not automatically make... allocations adopted" - working exactly as intended on real, not just test, data).

**Minor, non-blocking finding.** Bury's own document names JPA1.1 and JPA1.2 both "Northern Gateway" (Heywood/Pilsworth and Simister/Bowlee are two parts of one named allocation) - free-text geocoding failed for both, most likely due to the shared name being insufficiently specific for a place lookup. Not fixed in this sprint; noted as a Sprint 3 candidate (e.g. falling back to a borough-level centroid when a specific free-text geocode fails, rather than leaving a genuine allocation ungeocoded).

---

## 10. Addendum (Sprint 3E, "Joint Plan Support and Bury Allocation Reconciliation")

Two gaps this section had already flagged as future work were addressed:

**Joint-plan Council relationship.** This section's closing paragraph above noted "a future specification should still consider a proper many-to-many Council<->LocalPlan relationship for genuinely joint/multi-authority plans." That relationship now exists: an additive `LocalPlanCouncil` join table links one `LocalPlan` row to every Council it genuinely applies to, config-driven (`config/joint_plans.yaml`) rather than hardcoded. Places for Everyone remains exactly one `LocalPlan` row, now linked to all nine of its real participating authorities (confirmed against the plan's own Table 11.1, not inferred). `LocalPlan.council_code` is kept, unremoved, as a backwards-compatible lead/legacy field - every pre-Sprint-3E query that reads it directly still works; `app.policy.council_dashboard` and other council-facing views now read the join table (falling back to `council_code` for any plan not yet linked). See `app.policy.joint_plans` and `app.db.models.LocalPlanCouncil`.

**Same-named allocations across two plans are not automatically the same record.** Real Bury data surfaced exactly the risk this specification's relationship rules already guard against (§5): Bury's own Local Plan narratively lists Seedfield, Walshaw, Elton Reservoir, Castle Road (Unsworth) and Simister as sites Places for Everyone allocates, and - because the extraction pipeline still recorded these as the Bury Local Plan's own `LocalPlanSite` rows rather than recognising them as a cross-reference - each ended up duplicated (or, for Castle Road/Simister, contextually referenced with no allocation of their own) against the genuine Places for Everyone rows. An additive `AllocationRelationship` table now records this kind of cross-plan relationship (`same_physical_site`, `referenced_by`, etc.) without merging or deleting either row, and every classification is a human-reviewable `PolicyChangeEvent` proposal, never an automatic reclassification. See `app.policy.allocation_reconciliation` and `config/bury_allocation_reconciliation.yaml` for the specific, page-sourced findings this produced.
