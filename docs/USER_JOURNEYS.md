...
**Updated again at the Acquisition-First Roadmap Alignment:** buyer profiles, deterministic buyer fit, opportunity monitoring/change detection and entity-level Applicant Intelligence are now built and closed (Gate 1, Gate 1C, Gate 2A — see [PRODUCT_ROADMAP.md](PRODUCT_ROADMAP.md)), so each persona's journey now includes a real, evidenced "does this fit me, and has anything changed" step the sections below don't yet narrate explicitly — read every "Future workflow" below as sitting *after* that substrate, not before it. The platform's future primary scheme overview is being reoriented around five acquisition questions — see [PRODUCT_VISION.md](PRODUCT_VISION.md), "Acquisition-First UX Direction," and [PLATFORM_ARCHITECTURE.md](PLATFORM_ARCHITECTURE.md) for the underlying capabilities this depends on. Not a UI redesign in this update — recorded as direction the persona journeys below will need to reflect once Gate 2B/2C and the Acquisition Agent exist.
