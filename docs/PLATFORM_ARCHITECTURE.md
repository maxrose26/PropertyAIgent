...
**Acquisition Position Intelligence** (Gate 2C, after Gate 2B; purpose refined by the Acquisition Intelligence Architecture Amendment) is the next layer up: what the evidence establishes about the parties, control and transaction context of a development — known owner, applicant, developer/controller, promoter, delivery-partner and option/promotional-control evidence, construction/commencement evidence, explicit disposal/marketing evidence where it exists, and buyer-neutral Transaction/Disposition Signals. **Availability must never be inferred from silence, and Gate 2C never attempts to answer "is this site available?"** — see "Unknown Must Remain Unknown," [DESIGN_PRINCIPLES.md](DESIGN_PRINCIPLES.md), which governs this capability directly. `AVAILABLE`/`POTENTIALLY_AVAILABLE`/`NOT_AVAILABLE` are explicitly **rejected** as Gate 2C states — see [PRODUCT_ROADMAP.md](PRODUCT_ROADMAP.md), "Gate 2C — Acquisition Position, Transaction Signals & Buyer Acquisition Type," for the evidence-specific taxonomy (`APPLICANT_DECLARED_SOLE_OWNER`-style ownership facts, `DEVELOPER_APPLICANT_INDICATED`/`CONTROL_UNKNOWN`-style control facts) that replaces it — kept as a conceptually distinct state model from opportunity *type* (strategic land, long-pending application, …) and from a future buyer *recommendation* (`PURSUE / VERIFY / MONITOR / NOT RELEVANT`); these concepts must never be collapsed into one enum.
