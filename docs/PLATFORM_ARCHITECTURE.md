...
- **AI Allocation Intelligence Summary** — built and merged to master, production schema migrated; **not yet deployed and no real summary has yet been generated** (an explicit Product Owner four-allocation controlled sample is the next gate before any wider rollout — see "Allocation Intelligence" below and [PRODUCT_ROADMAP.md](PRODUCT_ROADMAP.md)). Automatic regeneration exists but ships disabled by default.
...
| 3. Allocation Intelligence | What is happening against this Local Plan allocation? | **Built, pending validation** — AI Allocation Intelligence Summary (see above); not yet deployed. |
| 4. Local Plan Delivery Intelligence | How are an authority's allocations progressing in practice, in aggregate? | **Planned** — see below; sequenced after Level 3 passes its Product Owner quality gate. |
| 5. Opportunity Intelligence | What commercially relevant land/development opportunities does the combined evidence indicate? | **Future direction** — the strategic destination this hierarchy is building toward, not a scheduled capability. Distinct from, and does not pull forward, the full investment/pursue-recommendation output already scoped under AI Decision Support (§5) — that still depends on Market Intelligence and Development Economics, unchanged. |
