...
2B-0A  APPLICATION FRESHNESS VERIFICATION   re-verify a known application's own record          [CLOSED]
        ↓
2B-1   SCHEME/APPLICATION/PHASE RECONCILIATION  which application/phase/version controls each fact [NEXT]
        ↓
...
- **Scheme / Application / Phase Reconciliation** (Gate 2B-1) — **NEXT**, gate definition approved 2026-09-10. Within an already-consolidated `Site`, which application/phase/proposal version is *eligible* to control each scheme-level acquisition fact (status, units, approved-vs-proposed, mix, affordable housing count/%, tenure, operative permission, current phase, scheme scope)? V1 is **deterministic, fact-level, computed/read-oriented** (no persisted operative-fact table — that is 2B-2), non-destructive, provenance-aware, and fail-safe (declines to state a value the evidence does not support). It resolves a `planning_role` per application — kept *distinct* from `application_category` (`application_category` = qualification/filtering; `planning_role` = lifecycle participation + which facts it may control), extending existing classifiers not duplicating them — then: resolve phase/scope (reuse `phase_tracking`) → determine per-fact eligibility → rank eligible sources → keep proposed/approved/superseded separate → surface unresolved same-scope conflicts (generalise `affordable_housing_scope`) → wire the presentation layers that currently depend on `pick_representative_application()`/`aggregate_scheme_fields()`. Hard non-substantive guardrail: an EIA screening opinion, condition-discharge filing, certificate, external consultation, ancillary or technical amendment cannot establish substantive permission or operative residential quantum. Two further safeguards (Product Owner, final approval): (a) **residential-only quantum is not a new inference** — resolve it only where existing evidence already supports a deterministic residential-vs-specialist distinction, otherwise keep the wider/all-use figure separate and decline to state a residential-only value (no new NLP/AI pass); (b) **S73/variation authority is fact-specific** — a later variation controls a fact only where evidence explicitly shows it varies that fact, never application-wide by recency. An explicit typed `ApplicationRelationship` is introduced only if a required acceptance case cannot be solved without it, and kept narrow. Driven by the real Astra regression cases (Burnage, Stockport Rugby, Pennington's Stables, World of Pets, Brixham Road, Hazelhurst Farm; London Road, Victoria House, Hopes Carr). Distinct from `app.pipeline.site_linking`'s existing physical-site consolidation.
