...
#### Planning Status Verification — MERGED TO MASTER (Gate 2B-0A), deployed fail-closed / dormant pending controlled rollout

`app.pipeline.status_verification` (merged to master at `8009c2a`) directly re-fetches an already-known Application's own authoritative portal record by exact reference — reusing, unmodified, `app.scrapers.{idox,arcus}_portal.fetch_application_by_reference` and `app.pipeline.run_weekly._upsert_scraped_application` (already generic, already runs `app.pipeline.material_change` detection) — rather than the existing month-range search, which can never revisit a past month, or the existing related-application search, which requires a granted anchor. Root cause this closes (Gate 2B-A investigation, production-confirmed): 173 pending applications >6 months old with zero refresh activity of any kind; 100% of `long_pending_application` opportunities >90 days unverified.
