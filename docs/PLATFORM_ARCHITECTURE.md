...
- **Scheme / Application / Phase Reconciliation** (Gate 2B-1) — **NEXT** (Product Owner re-sequenced ahead of lifecycle history at 2B-0A closure). Within an already-consolidated `Site`, which application/phase/proposal version controls each scheme-level acquisition fact (status, units, approved-vs-proposed, mix, affordable housing count/%, tenure, operative permission, current phase, scheme scope)? Deterministic fact-level operative-source selection with fact-level provenance — no single `current_application_id` assumed to control everything — plus a non-substantive-application guard (an EIA screening opinion, condition-discharge filing, or superseded/wider-site proposal can never become the source of substantive residential facts), and conflicts either resolved by stronger operative evidence or preserved and exposed. Proposed gate definition under Product Owner review; not yet approved. Distinct from `app.pipeline.site_linking`'s existing physical-site consolidation.
