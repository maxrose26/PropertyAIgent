...
**The availability principle (Product Owner decision, Acquisition Intelligence Architecture Amendment).** PropertyAIgent must never infer `AVAILABLE` / `POTENTIALLY_AVAILABLE` / `NOT_AVAILABLE` from indirect planning, ownership or development evidence — in most circumstances PropertyAIgent simply cannot know whether an owner is willing to sell. A site may only be described as explicitly `MARKETED` / `FOR SALE` / `DISPOSAL ACTIVE` where *direct* evidence supports it (active agent marketing, a formal disposal listing, a public-sector disposal notice, an auction listing, tender/process evidence). Absence of marketing evidence is not evidence of "not for sale," and apparently attractive circumstances (a mature, undeveloped consent; a stalled scheme) are not evidence of availability — they are, at most, a reason to *investigate*, never a factual claim of availability. Worked example: a developer secures permission for a 72-home scheme and, 30 months later, has still not commenced, with no marketing evidence known — PropertyAIgent must still never conclude `FOR SALE`; the correct, evidence-honest output is that a mature, developer-controlled, undeveloped consent may justify verifying the owner's current intentions, not that the site is available. This is the reason Gate 2C's own acquisition-position taxonomy (see [PRODUCT_ROADMAP.md](PRODUCT_ROADMAP.md), "Gate 2C — Acquisition Position, Transaction Signals & Buyer Acquisition Type") is built entirely from evidence-specific states, never a commercial-availability claim.
