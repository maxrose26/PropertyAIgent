...
**Suggested implementation order:**
1. **Allocated/Emerging Site Selection + Reporting** — the next immediate workstream now that Allocation Intelligence is pilot-complete: filtering/discovering allocated and emerging sites, selecting one or many into a shortlist, and generating a decision-ready report from that selection. The immediate next *gate* within this item is a **Site Selection & Reporting Architecture / Data Audit** (existing Planning Discovery filtering, existing planning-site reporting, existing Allocation Discovery filtering, what can be reused vs. needs building) — implementation architecture is only approved once that audit is complete; it has not yet started.
...
Beyond this list, Opportunity Intelligence (Intelligence Hierarchy Level 5, `PLATFORM_ARCHITECTURE.md` §2) is the strategic destination this sequence is building toward. Its own internal conceptual sequence — AI-assisted Opportunity Discovery, Planning Potential, general Opportunity Potential, then Buyer Profiles — is recorded in full in `PLATFORM_ARCHITECTURE.md` §2, but none of it is scheduled work, and nothing in this section is permission to start building any of it now.
