...
| Buyer Mandate V2 | **Phase A ✓ CLOSED (merged `d686c30`); Phase B1 (structured domain expansion) implemented + production-migrated, on feature branch, not yet merged; B2/B3 not started** | After Gate 2C | Extends Buyer Profiles V1 into a reusable, natural-language-inspectable acquisition mandate. Phase A: `Buyer`/`BuyerMandate` domain split. Phase B1: adds Geography/Acquisition Type/Development-State Appetite/Control Appetite as persisted, fingerprinted, NOT-yet-matching-active `BuyerMandate` fields — see "Buyer Mandate V2 — Phase B1" below. Legacy `BuyerProfile` table retained, read-only. B2 (wiring these into `assess_buyer_fit`) and B3 (mandate UX) require separate Product Owner instructions. |
