...
**2B-1 — Scheme / Application / Phase Reconciliation.** *(NEXT — proposed gate definition under Product Owner review; not yet approved for implementation.)* **Objective:** *"PropertyAIgent can determine which planning application, phase and proposal version should control each scheme-level acquisition fact, while preserving uncertainty and conflicts where the evidence does not support a definitive choice."* This is **not** duplicate-site consolidation (the platform already merges multiple planning references into one physical `Site` via `app.pipeline.site_linking`). 2B-1 addresses the different question: within an already-consolidated site, which application should determine current planning status, current/approved-vs-proposed residential unit count, house/apartment/other-use mix, affordable housing count and %, tenure, operative permission, current phase, and acquisition-relevant scheme scope. It introduces or formalises **deterministic fact-level operative-source selection** (no single application need supply every fact — status from the operative permission, detailed mix from latest approved reserved matters, AH terms from a later approved variation/S106, build status from condition-discharge/commencement evidence), with **fact-level provenance** rather than one `current_application_id` assumed to control everything, and a non-substantive-application guard so an EIA screening opinion, discharge-of-conditions filing, or superseded/wider-site proposal can never become the source of substantive residential facts. Where credible sources disagree, the conflict is either deterministically resolved by stronger operative evidence **or** preserved and exposed with provenance — never silently resolved for a clean UI. **Provisional exit condition:** *"For every planning-application opportunity, PropertyAIgent can state — or explicitly decline to state — which application/phase/version is operative for each scheme-level fact, and the resulting operative planning status, residential quantum and affordable-housing position, with provenance, evidence date, confidence and visible unresolved conflicts."* Designed around the real Astra regression cases (Former Burnage Cricket Club, Stockport Rugby Club, Pennington's Stables, World of Pets, Brixham Road, Hazelhurst Farm; also London Road, Victoria House, Hopes Carr). **This is an evidence/reconciliation gate — it explicitly does NOT introduce acquisition/buyer-fit/planning-readiness scoring, comparables, appraisal, valuation, alerts, or a dashboard redesign** (see decision below). **Prerequisite:** the completed Gate 2B repository & architecture investigation remains the design basis; the technical design is under Product Owner review and not yet approved.

**2B-2 — Operative Scheme Facts / Evidence Resolution.** *(AFTER 2B-1 — not yet defined.)* Consumes 2B-1's operative-source selection to publish the resolved operative fact set (homes, affordable housing, planning status, phase) with source, evidence date, confidence and visible conflicts — the "OPERATIVE OPPORTUNITY FACTS" layer in [PLATFORM_ARCHITECTURE.md](PLATFORM_ARCHITECTURE.md). Where the evidence does not justify an operative value, the system explicitly declines to state one. Scope and technical shape deliberately undecided until 2B-1's design is approved.
...
| Scheme / Application / Phase Reconciliation | **Proposed gate definition under Product Owner review** | Gate 2B-1 (NEXT) | Which application/phase/version controls each scheme-level fact; fact-level operative-source selection with provenance; non-substantive-application guard; conflicts resolved by stronger evidence or preserved |
...
**Gate 2B-1 — Scheme / Application / Phase Reconciliation.** Gate 2B-0A is closed (implemented, deployed at `4b98b46`, production-validated through the Oldham cohort). The proposed 2B-1 gate definition is under Product Owner review — objective, scope boundary, likely schema/domain changes, acceptance criteria against the real Astra regression cases, and explicit deferrals — and the Claude Code implementation prompt is not to be generated until that definition is approved. The completed Gate 2B repository & architecture investigation is the design basis; its findings are recorded in [PLATFORM_ARCHITECTURE.md](PLATFORM_ARCHITECTURE.md). Separately: platform-wide scheduled status verification stays fail-closed until the production daily-scrape scheduler is proven healthy (operational dependency, not Gate 2B feature work).
