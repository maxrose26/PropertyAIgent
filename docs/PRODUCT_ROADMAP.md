...
| Buyer Mandate V2 | **Phase A (domain separation) ✓ CLOSED — merged `d686c30`, production-validated; Phase B not started** | After Gate 2C | Extends Buyer Profiles V1 into a reusable, natural-language-inspectable acquisition mandate. Phase A: `Buyer` (identity) and `BuyerMandate` (persistent strategy, one buyer may hold several) are now separate domain objects — see "Buyer Mandate V2 — Phase A" below. Legacy `BuyerProfile` table retained, read-only, for rollback. Phase B (geography, Acquisition Type, control/development-state appetite) requires a separate Product Owner instruction. |
