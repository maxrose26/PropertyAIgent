...
| Buyer Mandate V2 | **Phase A ✓ CLOSED (merged `d686c30`); Phase B1 ✓ CLOSED (merged `44387fa`, production-validated); B2/B3 not started** | After Gate 2C | Extends Buyer Profiles V1 into a reusable, natural-language-inspectable acquisition mandate. Phase A: `Buyer`/`BuyerMandate` domain split. Phase B1: Geography/Acquisition Type/Development-State Appetite/Control Appetite persisted, fingerprinted, validated - deliberately inert in `assess_buyer_fit` until Phase B2. Legacy `BuyerProfile` table retained, read-only. B2 has two recorded mandatory entry conditions (matching-policy versioning; evidence-safe development-state interpretation) - see "Buyer Mandate V2 — Phase B1" below. B2/B3 require separate Product Owner instructions. |
