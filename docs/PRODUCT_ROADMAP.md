...
| Buyer Mandate V2 | **Investigated; Phase A (domain separation) implemented, on feature branch, not yet merged** | After Gate 2C | Extends Buyer Profiles V1 into a reusable, natural-language-inspectable acquisition mandate. Phase A: `Buyer` (identity) and `BuyerMandate` (persistent strategy) are now separate domain objects — see "Buyer Mandate V2 — Phase A" below. Phase B (geography, Acquisition Type, control/development-state appetite) not yet started. |
