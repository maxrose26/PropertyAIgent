...
| Planning freshness infrastructure | **Merged to master (`8009c2a`), deployed fail-closed / dormant — deployment + controlled rollout pending** | Gate 2B-0A (NEXT) | `Application.status_verified_at`, tiered direct re-verification, extended related-discovery eligibility; requires explicit `PLANNING_STATUS_VERIFICATION_ENABLED` activation |
...
**Verify the production deployment of master (`8009c2a`) and run the controlled one-authority Phase 2 rollout of Gate 2B-0A** (status verification is deployed dormant — activated for that run via `run_weekly --include-status-verification`, or platform-wide via `PLANNING_STATUS_VERIFICATION_ENABLED` once the Product Owner approves), then a Product Owner closure decision, before Gate 2B-0B's own design begins in earnest — the read-only Gate 2B repository & architecture investigation this once pointed to is complete, and its findings are recorded in [PLATFORM_ARCHITECTURE.md](PLATFORM_ARCHITECTURE.md).
