...
**2B-2 — Operative Scheme Facts / Evidence Resolution.** *(AFTER 2B-1 — architecture/design question, not yet decided.)* Formalises and exposes 2B-1's reconciled operative facts as a **stable, reusable layer** for UI, monitoring, buyer qualification and downstream consumers — the "OPERATIVE OPPORTUNITY FACTS" layer in [PLATFORM_ARCHITECTURE.md](PLATFORM_ARCHITECTURE.md). **Whether this needs persistence, a computed service/view, cached computation, versioning, or a hybrid is an open architecture decision** — the successful in-memory reconciliation of all seven real Astra Sites during Gate 2B-1 validation strengthens the computed-service case, but does not settle it. Must also address the carried-forward items above. Where the evidence does not justify an operative value, the system explicitly declines to state one. Scope and technical shape deliberately undecided.
...
**Gate 2B-2 — Operative Scheme Facts / Evidence Resolution (architecture/design question).** Gate 2B-0A is closed (deployed at `4b98b46`, Oldham-cohort validated). Gate 2B-1 is closed (merged `e51f74b`, 2026-09-10, read-only production-validated against seven Astra Sites — see "Carried-forward from Gate 2B-1"). The next question is whether/how to formalise 2B-1's reconciled operative facts as a stable reusable layer (persistence vs computed service vs cached vs versioned vs hybrid — undecided). No 2B-2 implementation is authorised yet. Separately: platform-wide scheduled status verification stays fail-closed until the production daily-scrape scheduler is proven healthy (operational dependency, not Gate 2B feature work).
